    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          <div key={s.label} className="bg-[#1a1a1a] border border-zinc-800/50 rounded-2xl p-4">
      ...
      {!extFilters && <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex gap-1">
          <button onClick={() => setFilterAssignee('all')}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${filterAssignee === 'all' ? 'text-black' : 'text-zinc-500 bg-[#1a1a1a]'}`}
        ...
        <div className="flex gap-1">
          {['all', 'Alta', 'Media', 'Baja'].map(p => (
            <button key={p} onClick={() => setFilterPriority(p)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${filterPriority === p ? 'text-black' : 'text-zinc-500 bg-[#1a1a1a]'}`}
        ...
        <div className="flex gap-1">
          {['all', 'todo', 'inprogress', 'done'].map(s => (
            <button key={s} onClick={() => setFilterStatus(s)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${filterStatus === s ? 'text-black' : 'text-zinc-500 bg-[#1a1a1a]'}`}
                ...
                <tr key={task.id} className="border-b border-zinc-800/50 hover:bg-white/[0.02] transition-colors group">
                  ...
                  <td className="px-4 py-3">
                    <select
                      ...
                      className="bg-[#111] border border-zinc-800 rounded-lg px-2 py-1 text-xs focus:outline-none focus:border-[#faff05]"
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div>
            ...
            <textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))}
              rows={2} className="w-full bg-[#1a1a1a] border border-zinc-800 rounded-xl px-3 py-2.5 text-white text-sm placeholder-zinc-700 focus:outline-none focus:border-[#faff05] resize-none"
