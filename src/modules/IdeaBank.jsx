    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-[#111] border border-zinc-800 rounded-2xl w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="flex items-center gap-2 px-3 py-2 bg-[#1a1a1a] rounded-xl border border-zinc-800">
          ...
          {clientsWithIdeas.length > 0 && (
            <div className="flex gap-1 flex-wrap">
              <button onClick={() => setFilterClient('all')}
                className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-all ${filterClient === 'all' ? 'text-black' : 'text-zinc-500 bg-[#1a1a1a] hover:text-white'}`}
              ...
              {clientsWithIdeas.map(c => (
                <button key={c.id} onClick={() => setFilterClient(filterClient === c.id ? 'all' : c.id)}
                  className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-all border-l-2 ${filterClient === c.id ? 'text-black' : 'text-zinc-500 bg-[#1a1a1a] hover:text-white'}`}
